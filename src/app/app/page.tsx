...
export default function HomePage() {
  ...
  const handleSubmit = () => {
    ...
    const label = attachments[0]?.name || "Yêu cầu paste văn bản.txt";
    loadSample(label);
    router.push("/app/processing");
  };

  return (
    <div className="container max-w-5xl px-4 py-5 sm:py-7 md:py-8">
      {/* Title */}
      ...
      <div className="relative">
        ...
        >
          ...
          <textarea
            ...
            onKeyDown={(e) => {
              // Enter (không Shift) = gửi; Shift+Enter = xuống dòng
          ...
          {/* Attachment chips — preview chờ gửi, 1 hàng scroll ngang (không đẩy textarea xuống) */}
      ...
      {/* Trust strip — compact */}
      <div className="mt-4 sm:mt-5 grid grid-cols-1 sm:grid-cols-3 gap-2">
        {[
          { icon: Bot, title: "Bóc tách nhiều định dạng", desc: "Excel, danh sách, hình ảnh.", color: "bg-violet-100 text-violet-600" },
          { icon: Clock, title: "Báo giá trong 60 giây", desc: "Kết nối tất cả NCC trên hệ thống.", color: "bg-blue-100 text-blue-600" },
          { icon: Boxes, title: "Đa dạng lựa chọn", desc: "Đa dạng sản phẩm, nhiều lựa chọn hơn.", color: "bg-amber-100 text-amber-600" },
        ].map((f, i) => (
          <div
            key={f.title}
            className="rounded-lg border bg-card px-2.5 py-2 flex items-center gap-2 opacity-0 animate-fade-up shadow-sm hover:shadow-md transition-shadow"
            style={{ animationDelay: `${100 + i * 80}ms` }}
          >
            <div className={cn("grid h-8 w-8 place-items-center rounded-md shrink-0", f.color)}>
      ...
      {/* Quy trình 4 bước — compact strip */}
      <div className="mt-4 rounded-xl border bg-gradient-to-r from-violet-50 via-white to-blue-50 px-3 sm:px-4 py-3">
        ...
        <ol className="flex items-center gap-1 sm:gap-2 overflow-x-auto">
          {["Tìm kiếm & bóc tách", "Chốt mã sản phẩm", "Kết nối NCC", "Đặt hàng & thanh toán"].map((s, i, arr) => (
            <React.Fragment key={s}>
              <li className="inline-flex items-center gap-1.5 shrink-0 px-2 py-1 rounded-md bg-white/70 border border-violet-100">
                <span className="grid h-5 w-5 place-items-center rounded-full brand-gradient text-white text-[10px] font-bold shrink-0">
                  {i + 1}
                </span>
                <span className="text-[12px] sm:text-xs font-medium text-violet-900 whitespace-nowrap">{s}</span>
              </li>
              {i < arr.length - 1 && (
...
/** Chip preview cho 1 attachment chờ gửi (giống ChatGPT/Messenger) */
